...
## Code Used in Hero image

Finally, for those who are curious, the code used to generate the hero image is below. For the sake of time, I got ChatGPT to write this one for me.

<Collapsible title="View Code" useMonoFont>
```lua
local TITLE  = "Lua as a Javascript Developer"
local FOOTER = "akif.kr"

-- find monitor: prefer arg side, else first monitor
local side = ({ ... })[1]
local mon = side and peripheral.wrap(side) or peripheral.find("monitor")
if not mon then error("No monitor found. Attach one or pass a side.") end

-- center helper
local function center(mon, y, text)
...
-- plain word-wrap to a given width
local function wrapText(s, width)
    local out = {}
    -- handle any embedded newlines just in case
    for line in s:gmatch("[^\n]+") do
        local rest = line
        while #rest > width do
            local chunk = rest:sub(1, width + 1)
            local cut = chunk:match(".*()%s+")  -- last space position (end idx)
            if cut and cut > 1 then
                table.insert(out, rest:sub(1, cut - 1))
                rest = rest:sub(cut + 1):gsub("^%s+", "")
            else
                table.insert(out, rest:sub(1, width))
...
-- choose a text scale that fits the title inside the frame
local function pickScale(mon, title)
    local scales = {5.0, 4.5, 4.0, 3.5, 3.0, 2.5, 2.0, 1.5, 1.0, 0.5}
    for _, sc in ipairs(scales) do
        mon.setTextScale(sc)
        local w, h = mon.getSize()
        local innerW, innerH = w - 4, h - 4
        if innerW >= 8 and innerH >= 3 then
            local lineCount = (#title <= innerW) and 1 or #wrapText(title, innerW)
            if lineCount <= innerH then return sc end
        end
    end
...
local function render(mon, title, footer)
    local w, h = mon.getSize()
    drawFrame(mon)
    local innerW, innerH = w - 4, h - 4
    local lines = (#title <= innerW) and { title } or wrapText(title, innerW)
    if #lines > innerH then
        while #lines > innerH do table.remove(lines) end
    end
    local topY = math.max(3, math.floor((h - #lines) / 2))
    for i, line in ipairs(lines) do
        center(mon, topY + i - 1, line)
    end
    if footer and #footer > 0 then
        local y = h
        local xRight = w - #footer
        if xRight > 1 then
            mon.setCursorPos(xRight, y); mon.write(footer)
        else
            mon.setCursorPos(2, y); mon.write(footer)
        end
    end
end
...
mon.setTextScale(pickScale(mon, TITLE))
render(mon, TITLE, FOOTER)
term.redirect(prev)
```
</Collapsible>