...
const copiedLabel = `<div class="flex items-center gap-1">${copiedCodeSvg}<div>Copied</div></div>`
...
for (const codeBlock of codeBlocks) {
    ...
    copyButton.className = "sticky top-2 right-2 text-white hover:text-white/75 px-3 py-2 text-xs leading-4 rounded z-10 pointer-events-auto";
